...
This directory holds raw Lido.js exports (`layers`) plus an optional, hand-authored
`meta` block. `meta` is **not** read by the Lido editor — it exists purely so
...
`template_227.json` is the canonical example. Read it alongside this file.

## How the Lido.js (template) flow uses this metadata

`POST /v1/lido/generate` (`app/lido_corpus/pipeline.py`) only ever fills a template that
has a `meta.reference_note` — that field is the "this template is ready" switch. It then
...
- `reference_note` — **makes the template eligible for `/v1/lido/generate`**; add it
  only once every slot's metadata is authored and checked. State the composition family
  in one sentence, then say explicitly what varies per brief (background subject/
  palette, cutout subject, text copy) versus what's structurally fixed (layer count,
  roles, the logo slot). It is for humans and selection, not sent to the model.
  Generated designs saved to `lido_generated/` have it cleared on purpose, so promoting
  one into this directory never makes it selectable until someone reviews it.

## Adding a new template

1. Drop the raw Lido export (`[{"layers": {...}}]`, no `meta`) into this directory.
2. Run `python scripts/enrich_lido_templates.py` to get a first-pass `meta` (LLM-backed
   `name`/`kind`/`tags`/`description` plus auto-computed geometry fields; per-slot
   `image`/`notes`/`max_chars`/`max_lines`/`locked` all start empty).
3. Hand-edit the per-slot fields following the rules above — this step is not
   automatable, it's where the actual design judgment goes. Reference
   `template_227.json` for the level of detail expected.
4. Re-run the enrichment script (or just load the template via the API) to confirm your
   hand-authored fields survive the recompute unchanged.
5. Check that the template's own copy passes its own limits with the real font — if it
   doesn't, the limits are wrong, not the copy:
...
6. Add `meta.reference_note`. From the next request on, the template is a candidate.
